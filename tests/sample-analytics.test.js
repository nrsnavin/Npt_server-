...
test('a sample takes its hook type from the tool it is made on', async () => {
  const customer = await api('/api/customers', {
    method: 'POST',
    token: nandhini,
    body: { assignedTo: await tokenOwnerId(nandhini), name: 'Hook Test Ltd', mobile: '9876533221' },
  });
...
test('what the request says beats what the register says', async () => {
  const customer = await api('/api/customers', {
    method: 'POST',
    token: nandhini,
    body: { assignedTo: await tokenOwnerId(nandhini), name: 'Override Ltd', mobile: '9876533222' },
  });
